...
module.exports = {
    name: "kanal-log",
    description: "Silinen veya oluşturulan kanalları görmenizi sağlar",
    type: 1,
    options: [
        {
            name: "kanal",
    ...
    run: async (client, interaction) => {
        ...
        const ayarlandi = new EmbedBuilder()
            .setColor("Green")
            .setDescription(`<:approve:1053645709997514803> Kanal log sistemi başarıyla ayarlandı **${interaction.user.username}**\n\n> #️⃣ ${kanal} __kanal log kanalı__ olarak ayarlandı`)

        interaction.followUp({ embeds: [ayarlandi] })
        louritydb.set(`kanalLog_${interaction.guild.id}`, kanal.id)
